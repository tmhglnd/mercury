...
**arguments**
- {Division} -> delaytime for left (default=3/16)
- {Division} -> delaytime for right (default=2/8)
- {Float+} -> feedback amount 0-1 (default=0.8)
- {Float+} -> lowpass filtering 0-1 (default=0.3)

```
fx(delay)
fx(delay <time-division>)
fx(delay <time> <feedback>)
fx(delay <time> <time> <feedback>)
fx(delay <time> <time> <feedback> <damping>)
```
