...
Create an FFT-freeze effect on the instruments sound. This will sound like the sound is frozen in time, but the timbre is still there. The freezer is triggered at the time interval of `time()` function. The trigger can be set with a float to give it a probability of triggering, or a ring can be provided to let the freezing occor in a rhythmic pattern. The second optional argument determines the sliding time between 2 freeze frames (in frames), and the third optional argument sets the amount of frames to store.
